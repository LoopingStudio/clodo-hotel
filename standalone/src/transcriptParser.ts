...
import type { AgentState, WebviewLike } from './types.js';
import {
	cancelWaitingTimer,
	startWaitingTimer,
	clearAgentActivity,
			...
			if (hasToolUse) {
				...
				for (const block of blocks) {
					if (block.type === 'tool_use' && block.id) {
						...
						webview?.postMessage({ type: 'agentToolStart', id: agentId, toolId: block.id, status });
					}
				}
				if (hasNonExemptTool) {
	...
	if (msgType === 'assistant') {
		...
		for (const block of content) {
			if (block.type === 'tool_use' && block.id) {
				...
				webview?.postMessage({ type: 'subagentToolStart', id: agentId, parentToolId, toolId: block.id, status });
			}
		}
