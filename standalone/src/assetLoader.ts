...
export function sendAssetsToWebview(webview: WebviewLike, assets: LoadedAssets): void {
	const spritesObj: Record<string, string[][]> = {};
	for (const [id, spriteData] of assets.sprites) spritesObj[id] = spriteData;
	webview.postMessage({ type: 'furnitureAssetsLoaded', catalog: assets.catalog, sprites: spritesObj });
}

export function sendWallTilesToWebview(webview: WebviewLike, wallTiles: LoadedWallTiles): void {
	webview.postMessage({ type: 'wallTilesLoaded', sprites: wallTiles.sprites });
}

export function sendFloorTilesToWebview(webview: WebviewLike, floorTiles: LoadedFloorTiles): void {
	webview.postMessage({ type: 'floorTilesLoaded', sprites: floorTiles.sprites });
}

export function sendCharacterSpritesToWebview(webview: WebviewLike, charSprites: LoadedCharacterSprites): void {
