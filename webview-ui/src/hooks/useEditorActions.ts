...
export function useEditorActions(
  ...
  // Called by useAppMessages on layoutLoaded to set the initial checkpoint
  ...
  const saveLayout = useCallback((layout: OfficeLayout) => {
    ...
    saveTimerRef.current = setTimeout(() => {
      appBridge.postMessage({ type: 'saveLayout', layout })
  ...
  const handleOpenClaude = useCallback(() => {
    appBridge.postMessage({ type: 'openClaude' })
  }, [])
